...
namespace Guagua
{
    public class ProcessMemoryReader
    {
        ...
        public void OpenProcess(uint _processID)
        {
            m_processID = _processID;
            m_hProcess = ProcessMemoryReaderApi.OpenProcess(ProcessMemoryReaderApi.PROCESS_VM_READ | ProcessMemoryReaderApi.PROCESS_VM_WRITE | ProcessMemoryReaderApi.PROCESS_VM_OPERATION, 1, (uint)m_processID);
        ...
        {
            IntPtr procHandle = ProcessMemoryReaderApi.OpenProcess(ProcessMemoryReaderApi.PROCESS_VM_READ | ProcessMemoryReaderApi.PROCESS_VM_WRITE | ProcessMemoryReaderApi.PROCESS_VM_OPERATION, 1, (uint)m_processID);
        ...
        public int ReadMultiLevelPointer(int MemoryAddress, uint bytesToRead, Int32[] offsetList)
        {
            ...
            for (int i = 0; i < (offsetList.Length); i++)
            {
                ...
                Console.WriteLine("pointerAddy>>{0}",pointerAddy);
                ProcessMemoryReaderApi.ReadProcessMemory(
                    procHandle,
                    (IntPtr)(pointerAddy),
                    btBuffer,
        ...
        public byte[] ReadAMem(int MemoryAddress, uint bytesToRead, out int bytesReaded)
        {
            byte[] buffer = new byte[bytesToRead];

            IntPtr ptrBytesReaded;
            ProcessMemoryReaderApi.ReadProcessMemory(m_hProcess, (IntPtr)MemoryAddress, buffer, bytesToRead, out ptrBytesReaded);
            bytesReaded = ptrBytesReaded.ToInt32();
            return buffer;
        }

        private byte[] readeDestBuffer = null;
        private uint buffersize = 0;
        public byte[] ReadtoMem(int MemoryAddress, uint bytesToRead, out int bytesReaded)
        {
            if (bytesToRead > buffersize)
            {
                readeDestBuffer = new byte[bytesToRead];
                buffersize = bytesToRead;
            }

            IntPtr ptrBytesReaded;
            ProcessMemoryReaderApi.ReadProcessMemory(m_hProcess, (IntPtr)MemoryAddress, readeDestBuffer, bytesToRead, out ptrBytesReaded);
            bytesReaded = ptrBytesReaded.ToInt32();
            return readeDestBuffer;
        }
